...
export function CartProvider({ children }) {
  ...
  // Re-verify coupon when cart changes (works for both auth and guest)
  useEffect(() => {
    const reVerifyCoupon = async () => {
      ...
      try {
        const cartTotal = Math.round(parseFloat(cart.subtotal || 0));
        const cartItemsPayload = (cart.items || []).map((item) => {
          const productId = item?.product?.id ?? item?.productId;
          const productVariantId = item?.variant?.id ?? item?.productVariantId;
          const brandId = item?.product?.brandId ?? item?.product?.brand?.id ?? null;
          const categoryIds = Array.isArray(item?.product?.categories)
            ? item.product.categories.map((c) => c?.id ?? c?.categoryId).filter(Boolean)
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cart.subtotal, cart.items?.length, coupon?.code]);
          ...
          items: prevCart.items.map((item) =>
            item.id === cartItemId
              ? {
                ...
                subtotal: Math.round(parseFloat(item.price) * quantity),
              }
              : item
          ),
      ...
      if (isAuthenticated) {
        ...
        setCart((prevCart) => {
          ...
          return {
            ...
            subtotal: Math.round(parseFloat(prevCart.subtotal) - itemSubtotal),
          };
        });
  ...
  // Build cart items payload for coupon verification (works for both server cart and guest cart)
  ...
  // Apply coupon (works for both authenticated and guest users)
  const applyCoupon = async (code) => {
    setCouponLoading(true);
    setError(null);
    try {
      const cartTotal = Math.round(parseFloat(cart.subtotal || 0));
      const cartItemsPayload = buildCartItemsPayload(cart.items);
      ...
      try {
        ...
        // For logged-in users, sync coupon to server cart (apply endpoint is protected)
        if (isAuthenticated) {
          fetchApi("/coupons/apply", {
            ...
            console.warn("Background coupon apply:", err);
          });
        }

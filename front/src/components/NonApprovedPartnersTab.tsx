...
export default function NonApprovedPartnersTab() {
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            const demoPassword = response.data.data.demoPassword || 'nutrybites';
    ...
    if (loading) {
        return <div className="text-center py-10 text-[var(--text-secondary)]">{t("partners_tab.common.loading")}</div>;
    }

    if (error) {
        return <div className="text-[var(--destructive)] text-center py-10">{error}</div>;
    }

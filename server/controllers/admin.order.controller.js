...
export const createOrder = asyncHandler(async (req, res, next) => {
  ...
  const order = await prisma.$transaction(async (tx) => {
    // Fetch coupon with couponPartners (multiple partners per coupon)
    let coupon = null;
    if (finalCouponId) {
      coupon = await tx.coupon.findUnique({
        where: { id: finalCouponId },
        include: { couponPartners: true },
      });
    }
    ...
    // If coupon has partners, create PartnerEarning for each (commission on order value after discount)
    if (coupon && coupon.couponPartners?.length > 0) {
      const commissionBase = subTotal - (discountAmount || 0);
      for (const cp of coupon.couponPartners) {
        if (cp.commission && cp.commission > 0 && commissionBase > 0) {
          const earningAmount = (commissionBase * cp.commission) / 100;
          await tx.partnerEarning.create({
            data: {

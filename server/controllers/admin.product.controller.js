...
export const createProduct = asyncHandler(async (req, res, next) => {
  ...
  try {
    const result = await prisma.$transaction(async (prisma) => {
      ...
      if (videoFile) {
        ...
        if (videoFile.size > maxVideoSize) {
          ...
          try {
            ...
            console.log(`✅ Product video uploaded: ${videoUrl}`);
          } catch (err) {
      ...
      const allFilesForVariants = Array.isArray(req.files)
        ? req.files
        : req.files
    ...
    try {
      const formattedProduct = {
        ...result,
        videoUrl: result.videoUrl ? getFileUrl(result.videoUrl) : null,
        // Extract categories into a more usable format
...
export const updateProduct = asyncHandler(async (req, res, next) => {
  ...
  try {
    const result = await prisma.$transaction(async (prisma) => {
      ...
      // Remove product video if requested
      if (req.body.removeVideo === "true" && product.videoUrl) {
        try {
          ...
          console.log("✅ Product video removed");
        } catch (err) {
          console.error("❌ Error removing product video:", err);
        }
      }
      ...
      if (videoFile) {
        ...
        if (videoFile.size > maxVideoSize) {
          ...
          try {
            ...
            console.log(`✅ Product video updated: ${videoUrl}`);
          } catch (err) {
            console.error("❌ Error uploading product video:", err);
          }
        }
...
export const reorderProductImages = asyncHandler(async (req, res, next) => {
  ...
  if (invalidIds.length > 0) {
    throw new ApiError(
      400,
      `Images ${invalidIds.join(", ")} do not belong to this product`
    );
  }
  ...
  try {
    ...
    res
      .status(200)
      .json(
        new ApiResponsive(200, {}, "Product images reordered successfully")
      );
  } catch (error) {
    console.error("Error reordering product images:", error);
    throw new ApiError(
      500,
      `Failed to reorder product images: ${error.message}`
    );
  }
});

...
export const getTopSellingProducts = asyncHandler(async (req, res) => {
  ...
  // Get product details
  const formattedTopSelling = await Promise.all(
...
// Get repeat products (products ordered multiple times)
export const getRepeatProducts = asyncHandler(async (req, res) => {
  const { limit = 10 } = req.query;

  // Find products that appear in the most unique orders
  const repeatProducts = await prisma.orderItem.groupBy({
    by: ["productId"],
    _count: {
      orderId: true,
    },
  ...
  const formattedRepeatProducts = await Promise.all(
    repeatProducts.map(async (item) => {
      const product = await prisma.product.findUnique({
        where: { id: item.productId },
        include: {
          images: {
      ...
      return {
        ...
        slug: product?.slug || "",
        image: product?.images[0] ? getFileUrl(product.images[0].url) : null,
        timesOrdered: item._count.orderId,
      };
    })
  );
  ...
  res.status(200).json(
    new ApiResponsive(
      200,
      { repeatProducts: formattedRepeatProducts },
      "Repeat products fetched successfully"
    )
  );
});

// Get repeat users (users with multiple orders)
export const getRepeatUsers = asyncHandler(async (req, res) => {
  const { limit = 10 } = req.query;

  // Find users with most orders
  const repeatUsers = await prisma.order.groupBy({
    by: ["userId"],
    _count: {
      id: true,
    },
  ...
  const formattedRepeatUsers = await Promise.all(
    repeatUsers.map(async (userStats) => {
      ...
      return {
        ...
        totalSpent: userStats._sum.total,
      };
    })
  );
  ...
  res.status(200).json(
    new ApiResponsive(
      200,
      { repeatUsers: formattedRepeatUsers },
      "Repeat users fetched successfully"
    )
  );
});

// Get user analytics (detailed user behavior)
export const getUserAnalytics = asyncHandler(async (req, res) => {
  ...
  const inactiveUsers = totalUsers - totalActiveUsers;
  ...
  res.status(200).json(
    new ApiResponsive(
      ...
      {
        totalUsers,
        totalActiveUsers, // Users who have ordered
        inactiveUsers, // Users who registered but haven't ordered
        usersWithCart,

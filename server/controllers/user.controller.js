...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  // Check if credentials user already exists (same email + credentials type)
  ...
  try {
    await sendEmail({
      email,
      subject: "Your OTP for Email Verification - nutrybites",
...
export const loginUser = asyncHandler(async (req, res, next) => {
  ...
  // Find credentials user (email + signupSource=credentials)
...
export const verifyOtp = asyncHandler(async (req, res, next) => {
  ...
  // OTP verification is for credentials signup only
  const user = await prisma.user.findFirst({
    where: { email, signupSource: "credentials" },
  });
  ...
  const { accessToken, refreshToken } = await generateAccessAndRefreshTokens(updatedUser.id);
  setCookies(res, accessToken, refreshToken);
  ...
  return res.status(200).json(
    new ApiResponsive(200, { user: userWithoutPassword, accessToken }, "Email verified successfully")
  );
});
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  // Forgot password is for credentials users only
  const user = await prisma.user.findFirst({
    where: { email, signupSource: "credentials" },
  });
  ...
  try {
    await sendEmail({
      email,
      subject: "Reset Your Password - nutrybites",
...
export const getCurrentUser = asyncHandler(async (req, res, next) => {
  try {
    ...
    // Remove sensitive data, keep signupSource for client (OAuth vs credentials)
    const userWithoutPassword = { ...user };
    delete userWithoutPassword.password;
    delete userWithoutPassword.otp;
    delete userWithoutPassword.otpVerifiedExpiry;
    userWithoutPassword.hasPassword = (user.signupSource ?? "credentials") === "credentials";
...
export const updateUserAddress = asyncHandler(async (req, res, next) => {
  ...
  // If setting as default, clear default flag from all other addresses
  if (isDefault) {
    await prisma.address.updateMany({
      where: {
        userId: req.user.id,
...
export const getUserWishlist = asyncHandler(async (req, res, next) => {
  try {
    ...
    // Format the response with improved image handling (apply flash sale)
    const formattedItems = await Promise.all(
      wishlistItems.map(async (item) => {
        ...
        const price = fs?.hasFlashSale ? fs.price : basePrice;
        const regularPrice = fs?.hasFlashSale ? fs.originalPrice : parseFloat(firstVariant?.price || 0);
...
export const requestAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Confirm Account Deletion - nutrybites",

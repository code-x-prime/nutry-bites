...
/**
 * Check if Shiprocket is enabled (from SiteSettings or ShiprocketSettings)
 */
export async function isShiprocketEnabled() {
    const siteSettings = await prisma.siteSettings.findFirst();
    if (siteSettings?.shiprocketEnabled && siteSettings?.shiprocketEmail && siteSettings?.shiprocketPassword) {
        return true;
    }
    const srSettings = await prisma.shiprocketSettings.findFirst();
    return !!(srSettings?.isEnabled && srSettings?.email && srSettings?.password);
}

/**
 * Get Shiprocket settings from database
 * Prefers SiteSettings when shiprocketEnabled; otherwise uses ShiprocketSettings
 */
export async function getShiprocketSettings() {
    ...
    if (siteSettings?.shiprocketEnabled && siteSettings?.shiprocketEmail && siteSettings?.shiprocketPassword) {
        return {
            isEnabled: true,
            email: siteSettings.shiprocketEmail,
            password: siteSettings.shiprocketPassword,
            token: siteSettings.shiprocketToken,
            tokenExpiry: siteSettings.shiprocketTokenExpiry,
            defaultLength: 10,
            defaultBreadth: 10,
            defaultHeight: 10,
            defaultWeight: 0.5,
...
export async function authenticate() {
    ...
    try {
        ...
        // Update token - use SiteSettings or ShiprocketSettings
        const siteSettings = await prisma.siteSettings.findFirst();
        if (siteSettings?.shiprocketEnabled && siteSettings?.shiprocketEmail) {
...
export async function processShiprocketReturn(orderId, returnReason = "Customer Return") {
    ...
    try {
        ...
        // Update order with return info (sync with Shiprocket)
        const returnOrderId = response.order_id ?? response.id ?? null;
        await prisma.order.update({
            where: { id: orderId },
            data: {
                shiprocketStatus: "RETURN_INITIATED",
                ...(returnOrderId != null && { shiprocketReturnId: parseInt(returnOrderId, 10) }),
            },
        });
